...
class LPIPS(nn.Module):
    ...
    def load_from_pretrained(self, name = "vgg_lpips"):
        ckpt = get_ckpt_path(name, "vgg.lpips")
        self.load_state_dict(torch.load(ckpt, map_location = torch.device("cpu")), strict = False)
        
    def forward(self, real_x, fake_x):
        # real과 fake 이미지를 vgg16에 넣어서 하나의 텐서값이 나오도록 도출
        # perceptual_loss에서 사용
        # print(real_x.size())  # [1, 3, 512, 512]
        # print(fake_x.size())  # [1, 3, 32, 32]
        features_real = self.vgg(self.scaling_layer(real_x))
        features_fake = self.vgg(self.scaling_layer(fake_x))

        diffs = {}
        
        # print('@@@@@@@@@@@@@@',features_real.size())
        # print('@@@@@@@@@@@@@@',features_fake.size())
        
        for i in range(len(self.channels)):
            
            # print('real size : ',norm_tensor(features_real[i]).size())
            # print('fake size : ',norm_tensor(features_fake[i]).size())
...
class ScalingLayer(nn.Module):
    def __init__(self):
        ...
        self.register_buffer("shift", torch.Tensor([-.030, -.080, -.188])[None, :, None, None])
        self.register_buffer("scale", torch.Tensor([.458, .488, .450])[None, :, None, None])
