...
class TrainVQGAN:
    def __init__(self, args):
        ...
        # if args.resume_from:
        #     self.vqgan = torch.load(args.resume_from)
        #     self.discriminator = Discriminator(ar)
    ...
    def prepare_training():
        os.makedirs("results_ver2", exist_ok=True)
        ...
        for epoch in range(0, args.epochs):
            with tqdm(range(len(train_dataset))) as pbar:
                for i, imgs in zip(pbar, train_dataset):
                    ...
                    perceptual_loss = self.perceptual_loss(imgs, decoded_images)  # decoded_images : fake
                    ...
                    if i % 10 == 0:
                        with torch.no_grad():
                            ...
                            vutils.save_image(real_fake_images, os.path.join("results_ver2", f"{epoch}_{i}.jpg"), nrow=3)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model-ckpt', type = str, default = '"distilbert-base-uncased"', help = 'Base model (default : distilbert)')
    ...
    args.dataset_path = r"/workspace/VQGAN/data/cat"
